...
class ConverstationCubit extends Cubit<ConverstationState> {
  ...
  Future<void> fetchConverstation({
    ...
    try {
      final result = await _chatRepository.getConverstation(
        parameter: {
  ...
  Future<void> fetchMore({
    ...
    if (state is ConverstationFetchSuccess) {
      ...
      try {
        emit(
          (state as ConverstationFetchSuccess)
              .copyWith(fetchMoreInProgress: true),
        );
        final moreMessages = await _chatRepository.getConverstation(
          parameter: {
            'type': isGroup ? 'group' : 'person',
            'to_id': toId,
            'from_id': fromUserId,
        ...
        final List<Message> messages = currentState.messages;
        for (final message in moreMessages['messages'] as List<Message>) {
  ...
  List<Message> getMessages() {
    if (state is ConverstationFetchSuccess) {
      ...
      chatMessages.sort(
        (a, b) => DateTime.parse(b.dateCreated!)
