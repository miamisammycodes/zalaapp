...
Future<bool> hasStoragePermissionGiven({String? fileExtension}) async {
  if (Platform.isIOS) {
    ...
    if (fileExtension != null) {
      ...
      if (["jpg", "jpeg", "png", "gif", "bmp", "webp", "heic", "heif"]
        ...
        return permissionGiven;
          ...
          .contains(ext)) {
        bool permissionGiven = await Permission.videos.isGranted;
        if (!permissionGiven) {
          return permissionGiven =
              (await Permission.videos.request()).isGranted;
        }
        return permissionGiven;
      } else if (["mp3", "wav", "aac", "ogg", "m4a", "flac"].contains(ext)) {
        bool permissionGiven = await Permission.audio.isGranted;
        if (!permissionGiven) {
          return permissionGiven = (await Permission.audio.request()).isGranted;
        }
        return permissionGiven;
      } else {
        // For PDFs and other files, use storage permission (for app-specific dirs)
        bool permissionGiven = await Permission.storage.isGranted;
        if (!permissionGiven) {
          return permissionGiven =
      ...
      // Default to storage permission if file type is unknown
      bool permissionGiven = await Permission.storage.isGranted;
      if (!permissionGiven) {
        return permissionGiven = (await Permission.storage.request()).isGranted;
      }
