...
class DatabaseHelper {
  ...
  Future<Database> initDb() async {
    ...
    if (!exists) {
      ...
      final ByteData data = await rootBundle.load(join("assets", "eShop.db"));
  ...
  addAndRemoveFav(String pid, {required bool isAdd}) async {
    final db1 = await db;
    if (isAdd) {
      addFavorite(pid);
    } else {
  ...
  addMostFav(String pid) async {
    ...
    if (result.length >= 10) {
      ...
      if (!check!) {
        db1.rawQuery(
          "DELETE FROM $MOSTFAV_TABLE WHERE $PID = ?",
          [result[result.length - 1][PID]],
        ).then((value) async {
  ...
  addMostLike(String pid) async {
    ...
    if (result.length >= 10) {
      ...
      if (!check!) {
        db1.rawQuery(
          "DELETE FROM $MOSTLIKE_TABLE WHERE $PID = ?",
          [result[result.length - 1][PID]],
        ).then((value) async {
  ...
  Future<bool> insertCart(
    ...
    if (check != "0") {
      ...
      if (getType == "0" ||
          (getType == "physical_product" && type == "physical_product") ||
          (getType == "digital_product" && type == "digital_product")) {
        final String query =
  ...
  Future<String?> checkCartItemTypeExists(String pid, String vid) async {
    ...
    if (result.isNotEmpty) {
      // Check if any item in cart is a digital product
      for (final item in result) {
        if (item[TYPE].toString() == "digital_product") {
  ...
  Future<List<String>?> getCart() async {
    ...
    final db1 = await db;
    final List<Map> result =
        await db1!.query(DatabaseHelper._instance.CART_TABLE);
    for (final row in result) {
      ids.add(row[VID]);
    }
    return ids;
