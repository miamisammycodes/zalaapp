...
class ManageAddress extends StatefulWidget {
  ...
  const ManageAddress({super.key, this.home, this.update, this.updateProgress});
  @override
...
class StateAddress extends State<ManageAddress> with TickerProviderStateMixin {
              ...
              Future.delayed(const Duration(seconds: 2)).then((_) async {
                ...
                if (_isNetworkAvail) {
                  ...
                  if (IS_SHIPROCKET_ON == "0") {
                    if (!ISFLAT_DEL) deliveryCharge = 0;
                  }
                  _getAddress();
                } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        backgroundColor: context.color.primarytheme,
        child: Icon(
          Icons.add,
  ...
  Future<void> setAsDefault(int index) async {
    try {
      ...
      apiBaseHelper.postAPICall(updateAddressApi, data).then(
        (getdata) {
          final bool error = getdata["error"];
