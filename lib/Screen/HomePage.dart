...
class HomePageState extends State<HomePage>
  ...
  Future<void> _refresh() {
    context.read<HomeProvider>().setCatLoading(loading: true);
    context.read<HomeProvider>().setSecLoading(loading: true);
    context.read<HomeProvider>().setOfferLoading(loading: true);
    context.read<HomeProvider>().setMostLikeLoading(loading: true);
    context.read<HomeProvider>().setSliderLoading(loading: true);
  ...
  Widget _slider() {
    ...
    final double height = deviceWidth! / 2.6;
        ...
        if (context.read<UserProvider>().userId != "") {
          ...
          apiBaseHelper.postAPICall(getFavApi, parameter).then(
            (getdata) {
              ...
              context.read<FavoriteProvider>().setLoading(isloading: false);
            },
            onError: (error) {
      ...
      apiBaseHelper.postAPICall(getSettingApi, parameter).then(
        (getdata) async {
          ...
          if (!error) {
            ...
            cartBtnList = data["cart_btn_on_list"] == "1";
            refer = data["is_refer_earn_on"] == "1";
            ...
            extendImg = data["expand_product_images"] == "1";
            ...
            try {
              ...
              if (popUpOffer.isActive == "1" &&
                  ...
                  popUpOffer.data is! List &&
                  popUpOffer.data.toString().isNotEmpty) {
  ...
  Widget _buildImagePageItem(Model slider) {
    final double height = deviceWidth! / 0.5;
    return InkWell(
      child: networkImageCommon(
        slider.image!,
        height,
  ...
  Future<void> _pincodeCheck() async {
    ...
    try {
      ...
      apiBaseHelper.postAPICall(getSliderApi, map).then(
        (getdata) {
          ...
          context.read<HomeProvider>().setSliderLoading(loading: false);
        },
        onError: (error) {
          setSnackbar(error.toString(), context);
          context.read<HomeProvider>().setSliderLoading(loading: false);
        ...
        (getdata) {
          ...
          if (!error) {
            ...
            if (getdata.containsKey("popular_categories")) {
              final data = getdata["popular_categories"];
              popularList =
                  (data as List).map((data) => Product.fromCat(data)).toList();
          ...
          context.read<HomeProvider>().setCatLoading(loading: false);
        },
        onError: (error) {
          setSnackbar(error.toString(), context);
          context.read<HomeProvider>().setCatLoading(loading: false);
