...
class SearchScreen extends StatefulWidget {
  ...
  const SearchScreen({super.key, this.catId});
  @override
...
class SearchScreenState extends State<SearchScreen>
  ...
  Widget listItem(int index) {
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: InkWell(
          splashColor:
              Theme.of(context).colorScheme.primarytheme.withValues(alpha: 0.2),
          onTap: () {
      ...
      if (context.read<UserProvider>().userId != "") {
        try {
          ...
          if (int.parse(qty) < productList[index].minOrderQuntity!) {
            String localqty = qty;
            localqty = productList[index].minOrderQuntity.toString();
            setSnackbar(
                "${getTranslated(context, 'MIN_MSG')}$localqty", context);
          }
          final parameter = {
    ...
    if (_isNetworkAvail) {
      if (context.read<UserProvider>().userId != "") {
        try {
          ...
          int qty;
          qty = int.parse(
                productList[index].prVarientList![model.selVarient!].cartCount!,
              ) -
  ...
  Future<void> getAvailVarient(List<Product> tempList) async {
    ...
    for (int j = 0; j < productList.length; j++) {
      final bool? check = await db.checkMostLikeExists(productList[j].id!);
      if (p < 5) {
        if (!check!) {
          p = p + 1;
      ...
      try {
        if (notificationisloadmore) {
          ...
          apiBaseHelper.postAPICall(getProductApi, parameter).then(
            (getdata) {
              final bool error = getdata["error"];
              final Map<String, dynamic> tempData = getdata;
              tagList.clear();
              if (tempData.containsKey(TAG)) {
                final List<String> tempList = List<String>.from(getdata[TAG]);
        ...
        builder: (BuildContext context, AsyncSnapshot<List<String>> snapshot) {
          if (snapshot.connectionState == ConnectionState.done &&
            ...
            for (int i = 0; i < entities.length; i++) {
              final Product item = Product.history(entities[i]);
              itemList.add(item);
            }
            history.clear();
  ...
  Future<void> initSpeechState() async {
    ...
    if (hasSpeech) {
      ...
      print("_localeNames:- $_localeNames");
  ...
  void startListening() {
    lastWords = '';
    speech.errorListener = (SpeechRecognitionError error) {
      print("Speech error: ${error.errorMsg}");
      speech.stop(); // Optional: Stop listening on error
    };
    ...
    return ListView.separated(
      ...
      itemBuilder: (BuildContext context, int i) {
        ...
        return ListTile(
          ...
          subtitle: textController!.text.trim().isEmpty || suggestion.history!
              ? null
              : Text(
          ...
          leading: textController!.text.trim().isEmpty || suggestion.history!
          ...
          onTap: () async {
            if (suggestion.name!.startsWith('Search Result for ')) {
              ...
              buildResult = false;
              currentHero = searchHero;
              final Product model = suggestion;
              Navigator.pushNamed(
                context,
                Routers.productDetails,
                arguments: {
