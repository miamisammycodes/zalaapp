...
class StateCart extends State<Cart> with TickerProviderStateMixin {
          ...
          builder: (context, data, child) {
            return _isNetworkAvailable
                ...
                : noInternet(
                    ...
                    onButtonClicked: ({required bool internetAvailable}) {
          ...
          children: [
            ...
            if (cartList[index].productList![0].productType !=
              ...
              if (IS_LOCAL_PICKUP != "1" || isStorePickUp != "true")
                (avail == false || avail == null) && deliverableList.isNotEmpty
                    ? Text(
                        deliveryMsg != ''
                            ? deliveryMsg
            ...
            if (!error) {
              ...
              if (cartList[0].productList![0].productType !=
                ...
                _getAddress(
                  ...
                  onInternetState: ({required bool hasInternet}) {
  ...
  Future<void> addToCart(
    ...
    if (_isNetworkAvailable) {
      try {
        ...
        if (int.parse(qty) < cartList[index].productList![0].minOrderQuntity!) {
          String localqty = qty;
          localqty = cartList[index].productList![0].minOrderQuntity.toString();
          setSnackbar("${getTranslated(context, 'MIN_MSG')}$localqty", context);
        }
        final parameter = {
  ...
  removeFromCartCheckout(int index, List<SectionModel> cartList,
  ...
  removeFromCart(int index, List<SectionModel> cartList, int selPos,
      {required bool remove, required bool move}) async {
    _isNetworkAvailable = await isNetworkAvailable();
    if (!remove &&
        int.parse(cartList[index].qty!) ==
            cartList[index].productList![0].minOrderQuntity) {
                          ...
                          child: SimBtn(
                            ...
                            onBtnSelected: () async {
                              ...
                              if (isPromoLen == false) {
                                ...
                                validatePromo(check: false).then((value) {
                                  FocusScope.of(context).unfocus();
                                });
                              }
                            },
  ...
  void _handlePaymentError(PaymentFailureResponse response) {
    ...
    if (errorMsg.isEmpty || errorMsg.toLowerCase() == 'undefined') {
  ...
  updateProgress({required bool progress}) {
    if (mounted) {
      checkoutState?.call(() {
        context.read<CartProvider>().setProgress(progress: progress);
      });
    }
  }
  ...
  Future<void> paytmPayment(
      String? tranId, String orderID, String? status, String? msg,
      {required bool redirect}) async {
    context.read<CartProvider>().setProgress(progress: true);
          ...
          if (!error) {
            ...
            if (response!['errorCode'] == null) {
              if (response['STATUS'] == 'TXN_SUCCESS') {
                ...
                addTransaction(response['TXNID'], orderId, SUCCESS, msg,
                    redirect: true);
              } else {
  ...
  Future<void> placeOrder(String? msg, {String? tranId}) async {
    _isNetworkAvailable = await isNetworkAvailable();
    if (_isNetworkAvailable) {
      try {
        context.read<CartProvider>().setProgress(progress: true);
        ...
        if (totalPrice > 0) {
          ...
          if (payVia == null && !isUseWallet!) {
            setSnackbar(getTranslated(context, 'PLZ_SEL_PAYMENT')!, context);
            context.read<CartProvider>().setProgress(progress: false);
            return;
          }
        } else {
          payVia = "COD"; // Default payment method for zero price orders
        }
        ...
        if (response.statusCode == 200) {
          ...
          if (!error) {
            ...
            if (paymentMethod == getTranslated(context, 'RAZORPAY_LBL')) {
              ...
              paystackPayment(context, tranId, orderId, SUCCESS, msg,
                  redirect: true);
            } else if (paymentMethod == getTranslated(context, 'PAYTM_LBL')) {
              paytmPayment(tranId, orderId, SUCCESS, msg, redirect: true);
            } else if (paymentMethod ==
                getTranslated(context, 'FLUTTERWAVE_LBL')) {
              flutterwavePayment(tranId, orderId, SUCCESS, msg, redirect: true);
            } else if (paymentMethod ==
  ...
  paystackPayment(BuildContext context, String? tranId, String orderID,
      String? status, String? msg,
      {required bool redirect}) async {
    context.read<CartProvider>().setProgress(progress: true);
    await paystackPlugin.initialize(publicKey: paystackId!);
    print('AmountPrintHere:-$totalPrice');
    ...
    try {
      ...
      print('Printing the charge:- $status');
      if (response.status) {
        addTransaction(response.reference, orderID, SUCCESS, msg,
            redirect: true);
      } else {
        deleteOrders(orderID);
  ...
  midTrasPayment(String orderID, String? status, String? msg,
      {required bool redirect}) async {
    _isNetworkAvailable = await isNetworkAvailable();
    if (_isNetworkAvailable) {
      try {
  ...
  fatoorahPayment(String? tranId, String orderID, String? status, String? msg,
      {required bool redirect}) async {
    _isNetworkAvailable = await isNetworkAvailable();
    if (_isNetworkAvailable) {
      try {
  ...
  Future<void> validatePromo({required bool check}) async {
    _isNetworkAvailable = await isNetworkAvailable();
    if (_isNetworkAvailable) {
      try {
        context.read<CartProvider>().setProgress(progress: true);
        ...
        apiBaseHelper.postAPICall(validatePromoApi, parameter).then(
          (getdata) {
            ...
            if (!error) {
              ...
              totalPrice = double.parse(data["final_total"].toString());
              promoAmount = double.parse(data["final_discount"].toString());
  ...
  Future<void> checkDeliverable(
    ...
    if (_isNetworkAvailable) {
      try {
        ...
        // If store pickup is selected, set deliverable to true and return
