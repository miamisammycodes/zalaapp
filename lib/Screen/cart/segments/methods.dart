...
Widget noInternet(
  ...
  required Function({required bool internetAvailable}) onButtonClicked,
}) {
        ...
        AppBtn(
          ...
          onBtnSelected: () async {
            ...
            Future.delayed(const Duration(seconds: 2)).then((_) async {
              ...
              onButtonClicked.call(internetAvailable: isNetworkAvail);
            });
          },
        ),
      ],
...
Future<void> _getAddress(
  ...
  required Function({required bool hasInternet}) onInternetState,
}) async {
  final bool NetworkAvailable = await isNetworkAvailable();
  if (NetworkAvailable) {
    onInternetState.call(hasInternet: true);
    try {
      final parameter = {
    ...
    onInternetState.call(hasInternet: false);
  }
}
...
_imgFromGallery(
  BuildContext context, {
  required Function(List<File> pickedFiles) onFilePick,
}) async {
  try {
