...
class StateWallet extends State<MyWalletScreen> with TickerProviderStateMixin {
  ...
  withDrawDailog() {
    ...
    return dialogAnimate(
      ...
      AlertDialog(
        ...
        title: Text(
          getTranslated(context, 'SEND_WITHD_REQ_LBL')!,
  ...
  _showDialog() async {
    ...
    // Reset payment method selection when dialog opens
    selectedMethod = null;
    payMethod = null;
    for (final element in payModel) {
      element.isSelected = false;
                ...
                Form(
                  ...
                  child: Flexible(
                    child: SingleChildScrollView(
                      key: ValueKey(
                          'form_${amtC?.text.hashCode}_${msgC?.text.hashCode}'),
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        mainAxisSize: MainAxisSize.min,
                        children: <Widget>[
                          ...
                          Padding(
                            padding:
                                const EdgeInsets.fromLTRB(20.0, 10, 20.0, 5),
                            child: Row(
  ...
  Future<void> phonepePayment(String amount) async {
    try {
      final bool isPhonePeInitilized = await PhonePePaymentSdk.init(
      ...
      if (response != null) {
        ...
        if (status == 'SUCCESS') {
          setSnackbar(
            getTranslated(context, 'PHONEPE_PAYMENT_SUCCESS')!,
            context,
          );
          return;
        }
  ...
  paystackPayment(BuildContext context, int price) async {
    ...
    final String email = context.read<UserProvider>().email;
  ...
  Future<void> getWithdrawalTransaction() async {
    ...
    if (_isNetworkAvail) {
      try {
        ...
        apiBaseHelper.postAPICall(getWithdrawReqApi, parameter).then(
          (getdata) {
            final bool error = getdata["error"];
            if (!error) {
  ...
  Future<void> getTransaction() async {
    ...
    if (_isNetworkAvail) {
      try {
        ...
        apiBaseHelper.postAPICall(getWalTranApi, parameter).then(
          (getdata) {
            final bool error = getdata["error"];
            if (!error) {
      ...
      try {
        ...
        apiBaseHelper.postAPICall(getSettingApi, parameter).then(
          (getdata) async {
            ...
            if (dialogState != null) dialogState?.call(() {});
          },
          onError: (error) {
            setSnackbar(error.toString(), context);
          },
  ...
  // Custom validator for amount field to check for zero
  String? validateAmount(String? value, String? msg) {
    if (value == null || value.isEmpty) {
      return msg;
    }
    final amount = double.tryParse(value);
    if (amount == null || amount <= 0) {
      return getTranslated(context, 'AMOUNT_GREATER_THAN_ZERO') ??
          'Amount must be greater than 0';
    }
    return null;
  }

  // Force clear form fields (more explicit)
  void forceClearFormFields() {
    print("forceClearFormFields called");

    // Clear controllers with null checks
    if (amtC != null) amtC!.text = '';
    if (msgC != null) msgC!.text = '';
    if (amtC1 != null) amtC1!.text = '';
    if (bankDetailC != null) bankDetailC!.text = '';

    print("Controllers cleared: amtC=${amtC?.text}, msgC=${msgC?.text}");
    ...
    // Force rebuild only if widget is still mounted
    if (mounted) {
      setState(() {});
    }

    // Update dialog if open and widget is mounted
    if (dialogState != null && mounted) {
      dialogState!(() {});
    }

    print("forceClearFormFields completed");
  }
