...
class StateItem extends State<ProductDetail> with TickerProviderStateMixin {
        ...
        for (int i = 0; i < model.reviewList![0].productRating!.length; i++) {
          for (int j = 0;
            ...
            final ImgModel m = ImgModel.fromJson(
              i,
              model.reviewList![0].productRating![i].imgList![j],
            );
            revImgList.add(m);
          }
        }
    ...
    if (proIds1.isNotEmpty) {
      ...
      if (_isNetworkAvail) {
        try {
          ...
          apiBaseHelper.postAPICall(getProductApi, parameter).then(
            (getdata) async {
              ...
              if (!error) {
                ...
                final extPro =
                    tempList.firstWhereOrNull((cp) => cp.id == model.id);
                if (extPro == null) {
              ...
              if (mounted) {
                setState(() {
                  context
                      .read<HomeProvider>()
                      .setMostLikeLoading(loading: false);
                });
              }
            },
            onError: (error) {
          ...
          context.read<HomeProvider>().setMostLikeLoading(loading: false);
        }
      } else {
        if (mounted) {
          setState(() {
            ...
            child: PageView.builder(
              ...
              itemBuilder: (BuildContext context, int i) {
                return sliderList[i] != "youtube"
                    ? networkImageCommon(sliderList[i]!, height, isSlider: true)
                    : playIcon(data);
              },
            ),
          ),
  ...
  void loadMoreComPro() {
    setState(() {
      context.read<ProductDetailProvider>().setProNotiLoading(loading: true);
                                  ...
                                  return ListItemCom(
                                    ...
                                    isSelected: (bool value) {
                                      setState(() {
                                        if (value) {
                                          final extPro =
                                              compareList.firstWhereOrNull(
                                            (cp) => cp.id == data[index].id,
                                          );
                                          if (extPro == null) {
                                            context
          ...
          onTap: () {
            if (context.read<UserProvider>().userId != "") {
              ...
              if (!data.contains(model.id)) {
                ...
                db.addAndRemoveFav(model.id!, isAdd: true);
                ...
                db.addAndRemoveFav(model.id!, isAdd: false);
                model.isFavLoading = false;
              }
              setState(() {});
            }
              ...
              if (attSType[i] == '1') {
                ...
                itemLabel = ClipRRect(
                  ...
                  child: Image.network(
                    ...
                    width: 80,
                    height: 80,
                    cacheHeight: 300, // limits image resolution in memory
                      ...
                      if (mounted) {
                        setState(() {
                          ...
                          for (int i = 0;
                              ...
                              i++) {
                            sinId = model.prVarientList![i].attribute_value_ids!
                                .split(',');
                    ...
                    if (available!) {
                      if (context.read<UserProvider>().userId != "") {
                        ...
                        if (qty == "0") {
                          ...
                          model.prVarientList![_oldSelVarient].cartCount = qty;
                          qtyController.text = qty;
                          qtyChange = true;
                        }
                      }
                                ...
                                onBtnSelected: () async {
                                  if (validateAndSave()) {
                                    if (IS_SHIPROCKET_ON == "1") {
                                      ...
                                      validatePin(curPin, first: false, data);
                                    }
                                  }
                                },
                              ),
  ...
  addAndRemoveQty(
    ...
    if (context.read<UserProvider>().userId != "") {
      ...
      if (context.read<CartProvider>().cartList.any(
            (element) =>
      ...
      context.read<CartProvider>().setProgress(progress: false);
      setState(() {});
    } else {
      if (from == 1) {
        if (int.parse(qty) >= totalLen) {
  ...
  Future<void> addToCart(String qty, Product product,
      {required bool intent, required bool from}) async {
    try {
      _isNetworkAvail = await isNetworkAvailable();
      if (_isNetworkAvail) {
        ...
        if (context.read<UserProvider>().userId != "") {
          try {
            if (mounted) {
              setState(() {
                context.read<CartProvider>().setProgress(progress: true);
              });
            }
            if (int.parse(qty) < model1.minOrderQuntity!) {
              String localeqty = qty;
              localeqty = model1.minOrderQuntity.toString();
              setSnackbar(
                  "${getTranslated(context, 'MIN_MSG')}$localeqty", context);
            }
            final parameter = {
            ...
            apiBaseHelper.postAPICall(manageCartApi, parameter).then(
              (getdata) {
                ...
                if (!error) {
                  final data = getdata["data"];
                  model1.prVarientList![_oldSelVarient].cartCount = qty;
                  if (from) {
                    context
                        .read<UserProvider>()
                    ...
                    if (intent) {
                      ...
                      setSnackbar(
                        getTranslated(context, 'PRO_ADD_TO_CART_LBL')!,
                        context,
                      );
                    }
                  }
  ...
  postQues(Product data) {
    return Padding(
      ...
      child: Column(
        ...
        children: <Widget>[
          Text(
            getTranslated(context, 'HAVE_DOUBTS_REG_THIS_PRO_LBL')!,
            style: TextStyle(
          ...
          Padding(
            ...
            child: CupertinoButton(
              ...
              child: Container(
                ...
                child: Text(
                  getTranslated(context, 'POST_YR_QUE_LBL')!,
                  textAlign: TextAlign.center,
                  style: Theme.of(context).textTheme.titleSmall!.copyWith(
                        color: Theme.of(context).colorScheme.fontColor,
                        fontWeight: FontWeight.bold,
        ...
        : faqsProductList.isNotEmpty
            ? Padding(
                ...
                child: Column(
                  ...
                  children: [
                    Text(
                      getTranslated(context, 'QUE_ANS_LBL')!,
                      style: Theme.of(context).textTheme.titleSmall!.copyWith(
                            color: Theme.of(context).colorScheme.fontColor,
                            fontWeight: FontWeight.bold,
                          ),
            ...
            if (!error) {
              ...
              if (mounted) {
                setState(() {
                  context
                      .read<ProductDetailProvider>()
                      .setProNotiLoading(loading: false);
                });
              }
            }
          },
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            if (model.desc!.isNotEmpty)
                              ...
                              const SizedBox(),
                            _attr(data),
                            if (model.madein != "" && model.madein!.isNotEmpty)
                              const Divider()
                            else
                              const SizedBox(),
                            _madeIn(data),
              ...
              onTap: () async {
                if (isCityWiseDelivery == true) {
                  ...
                  if (cityResponse != null) {
                    ...
                    context.read<HomeProvider>().setSecLoading(loading: false);
                    validatePin(pincodeOrCityName ?? "", first: true, data);
                    setState(() {});
                  }
                  return;
                } else {
              ...
              children: [
                if (codDeliveryCharges != '')
                  Padding(
                    ...
                    child: Row(
                      children: [
                        Text(
                          "${getTranslated(context, 'COD_CHARGE_LBL')}: ",
                          style: TextStyle(
                            color: Theme.of(context).colorScheme.lightBlack2,
                          ),
                        ),
                ...
                if (prePaymentDeliveryCharges != '')
                  Row(
                    children: [
                      Text(
                        '${getTranslated(context, 'ONLINE_PAY_LBL')}: ',
                        style: TextStyle(
                          color: Theme.of(context).colorScheme.lightBlack2,
                        ),
                      ),
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            if (gaurantee != "" && gaurantee!.isNotEmpty)
              Expanded(
                child: Padding(
                  padding: const EdgeInsetsDirectional.only(start: 7.0),
                  child: Column(
            ...
            if (warranty != "" && warranty!.isNotEmpty)
              Expanded(
                child: Padding(
                  padding: const EdgeInsetsDirectional.only(start: 7.0),
                  child: Column(
        ...
        children: [
          ...
          InkWell(
            ...
            onTap: () async {
              ...
              // Refresh product data when returning from review list
              if (mounted) {
                await getReview();
                setState(() {});
              }
  ...
  playIcon(Product data) {
    ...
    return Align(
      child:
          (model.videType != "" && model.video!.isNotEmpty && model.video != "")
              ? Icon(
  ...
  Future<void> validatePinFromShipRocket(String pin, Product data,
      {required bool wantsToPop}) async {
    try {
      _isNetworkAvail = await isNetworkAvailable();
      if (_isNetworkAvail) {
        try {
  ...
  Future<void> validatePin(String pinOrCity, Product data,
      {required bool first}) async {
    try {
      _isNetworkAvail = await isNetworkAvailable();
      if (_isNetworkAvail) {
        try {
            ...
            (getdata) {
              ...
              if (error) {
                ...
                if (pinOrCity != context.read<UserProvider>().curPincode) {
                  context.read<HomeProvider>().setSecLoading(loading: true);
                  getSection();
                }
                setState(() {});
                deliveryMsg = msg!;
  ...
  void getSection() {
    ...
    apiBaseHelper.postAPICall(getSectionApi, parameters).then(
      (getData) {
        final bool error = getData["error"];
        final String? msg = getData["message"];
        ...
        if (!error) {
          ...
          featuredSections.addAll(
              (data as List).map((data) => SectionModel.fromJson(data)));
        } else {
          if (userProvider.curPincode.isNotEmpty) userProvider.setPincode('');
          setSnackbar(msg!, context);
        }
        homeProvider.setSecLoading(loading: false);
      },
      onError: (error) {
        setSnackbar(error.toString(), context);
        homeProvider.setSecLoading(loading: false);
  ...
  Future<void> getDeliverable(Product data) async {
    ...
    if (pin != '') {
      if (IS_SHIPROCKET_ON == "1") {
        validatePinFromShipRocket(pin, wantsToPop: false, data);
      } else if (isCityWiseDelivery!) {
        validatePin(pin, first: true, data);
      }
    }
  }
  ...
  Future<void> setFaqsQue(Product data) async {
    ...
    if (_isNetworkAvail) {
      try {
        ...
        apiBaseHelper.postAPICall(setProductFaqsApi, parameter).then(
          (getdata) {
            final bool error = getdata["error"];
            final String? msg = getdata["message"];
            Navigator.pop(context);
