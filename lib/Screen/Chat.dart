...
class ChatState extends State<Chat> {
  ...
  Widget MsgContent(int index, Model message) {
    ...
    return Column(
      ...
      children: <Widget>[
        if (message.uid == settingsProvider.userId)
        ...
        if (message.msg != null && message.msg!.isNotEmpty)
          Card(
            ...
            color: message.uid == settingsProvider.userId
                ? Theme.of(context).colorScheme.fontColor.withValues(alpha: 0.1)
  ...
  Future<void> _requestDownload(String? url, String? mid) async {
    ...
    final bool checkPermission =
        await hasStoragePermissionGiven(fileExtension: fileExtension);
    if (checkPermission) {
      if (Platform.isIOS) {
        final Directory target = await getApplicationDocumentsDirectory();
        _filePath = target.path;
      } else {
        _filePath = await getExternalStoragePath();
      }
      ...
      if (downloadlist.containsKey(mid)) {
        ...
        final taskId = downloadlist[mid]!;
        final matches = allTasks!.where((t) => t.taskId == taskId).toList();
        if (matches.isNotEmpty) {
          final task = matches.first;
          if (task.status == DownloadTaskStatus.complete ||
              task.status == DownloadTaskStatus.failed) {
            downloadlist.remove(mid);
          }
        }
      }
      if (hasExisted) {
        setSnackbar("File already downloaded", context);
  ...
  Future<bool> _checkPermission({String? fileExtension}) async {
    // Deprecated: use hasStoragePermissionGiven instead
    return hasStoragePermissionGiven(fileExtension: fileExtension ?? "pdf");
  }
  ...
  Future<void> getMsg() async {
    try {
      ...
      apiBaseHelper.postAPICall(getMsgApi, data).then(
        (getdata) {
          final bool error = getdata["error"];
