...
class FlashSaleState extends State<FlashSale> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    final offerList = context.watch<OfferImagesProvider>().offerList;
    final saleList = context.watch<FlashSaleProvider>().saleList;
    final bool showNoOffers =
        !_loading && offerList.isEmpty && saleList.isEmpty;
              ...
              child: showNoOffers
                  ? Center(
                      child: Padding(
                        ...
                        child: Text(
                          getTranslated(context, "NO_AVAILABLE_AT_THE_MOMNET")!,
    ...
    return _loading
        ...
        : Selector<OfferImagesProvider, List<SliderImages>>(
            builder: (context, offerData, child) {
              final saleData = context.watch<FlashSaleProvider>().saleList;
              if (offerData.isEmpty && saleData.isEmpty) {
                return Padding(
                  padding: const EdgeInsets.only(top: 40.0),
                  child: Center(
                    child: Text(
              ...
              return Padding(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    if (offerData[index].style == "default" ||
                        offerData[index].style == "style_1") {
                      context
                          .read<OfferImagesProvider>()
                          .setOfferCurSlider(index);
                    }
                    return imageSliderWidget(index, offerData);
                  },
                ),
              );
            },
                    ...
                    ? SizedBox(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            return InkWell(
                              onTap:
                                  data[i].offerImages![index].type != "default"
                                      ? () {
                                          onOfferImageClick(i, data, index);
                                        }
                                      : null,
  ...
  void getFlashSale() {
    try {
      apiBaseHelper.postAPICall(getFlashSaleApi, {}).then(
        (getdata) {
  ...
  void getOfferImages() {
    ...
    try {
      apiBaseHelper.postAPICall(getOfferImageApi, {}).then(
        (getdata) {
          final bool error = getdata["error"];
          if (mounted) {
