...
class LoginPageState extends State<LoginScreen> with TickerProviderStateMixin {
  final mobileController =
      TextEditingController(text: isDemoApp ? "8989898989" : "");
  final passwordController =
      TextEditingController(text: isDemoApp ? "Demo@1234" : "");
  String? countryName;
  FocusNode? passFocus;
  ...
  void getSetting() {
    try {
      apiBaseHelper.postAPICall(getSettingApi, {}).then(
        (getdata) async {
    ...
    apiBaseHelper.postAPICall(getUserLoginApi, data).then(
      (getdata) async {
        ...
        if (!error) {
          ...
          settingProvider.setPrefrenceBool(ISFIRSTTIME, value: true);
                ...
                child: InkWell(
                  ...
                  onTap: () async {
                    if (acceptTnC) {
                      ...
                      setSnackbar(
                        getTranslated(context, 'agreeTCFirst')!,
                        context,
                      );
                    }
                  },
