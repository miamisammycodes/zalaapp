...
class AddAddress extends StatefulWidget {
  ...
  const AddAddress({super.key, this.update, this.index, this.updateState});
  @override
...
class StateAddress extends State<AddAddress> with TickerProviderStateMixin {
  ...
  TextEditingController? landmarkC;
  TextEditingController? stateC;
  TextEditingController? countryC;
  TextEditingController? altMobC;
  TextEditingController? cityC;
  TextEditingController? areaC;
  int? selectedType = 1;
  ...
  bool pincodeEnable = false;
  bool? isLoadingMoreCity;
  bool? isLoadingMorePincode;
  @override
  void initState() {
    super.initState();
    ...
    if (widget.update!) {
      final User item = addressList[widget.index!];
      ...
      checkedDefault = item.isDefault == '1';
    } else {
      getCurrentLoc();
    }
  }
  ...
  _scrollListener() async {
    if (_cityScrollController.offset >=
      ...
      if (mounted) {
        ...
        if (cityOffset < cityTotal) getCities(isSearchCity: false);
      }
    }
  }
  ...
  _pincodeScrollListener() async {
    if (_pinCodeScrollController.offset >=
      ...
      if (mounted) {
        ...
        getPinCode(city, isSearchArea: false, clear: false);
      }
    }
  }
                        ...
                        Padding(
                          ...
                          child: IconButton(
                            onPressed: () async {
                              ...
                              await getPinCode(city,
                                  isSearchArea: true, clear: true);
                              setState(() {});
                            },
                            icon: const Icon(
                              Icons.search,
                          ...
                          child: SingleChildScrollView(
                            ...
                            child: Column(
                              children: [
                                ...
                                if (pincodeSearchList.isNotEmpty)
                                  Column(
                                    crossAxisAlignment:
                                ...
                                showCircularProgress(
                                  context,
                                  isProgress: isLoadingMorePincode!,
                                  Theme.of(context).colorScheme.primarytheme,
                                ),
                              ],
                            ),
                      ...
                      Padding(
                        ...
                        child: IconButton(
                          onPressed: () async {
                            ...
                            setState(() {});
                            await getCities(isSearchCity: true);
                          },
                          icon: const Icon(
                            Icons.search,
                            size: 20,
            ...
            InkWell(
              onTap: () {
                if (mounted) {
                  ...
                  getPinCode(city, isSearchArea: true, clear: true);
                  setState(() {});
                }
              },
              child: SizedBox(
  ...
  Future<void> callApi() async {
    final bool avail = await isNetworkAvailable();
    if (avail) {
      await getCities(isSearchCity: false);
      if (widget.update! && addressList[widget.index!].cityId != "0") {
        getPinCode(addressList[widget.index!].cityId,
            isSearchArea: false, clear: false);
      }
    } else {
      Future.delayed(const Duration(seconds: 2)).then((_) async {
        if (mounted) {
  ...
  Future<void> getCities({required bool isSearchCity}) async {
    try {
      final parameter = {
      ...
      apiBaseHelper.postAPICall(getCitiesApi, parameter).then(
        (getdata) async {
  ...
  Future<void> getPinCode(String? city,
      {required bool clear, required bool isSearchArea}) async {
    try {
      final data = {
        ID: city,
      ...
      apiBaseHelper.postAPICall(getZipCodeByCityApi, data).then(
        (getdata) {
          final bool error = getdata['error'];
          final String? msg = getdata['message'];
          if (!error) {
            ...
            if (widget.update!) {
              for (final User item in addressList) {
      ...
      await apiBaseHelper
        ...
        (getdata) async {
          ...
          if (!error) {
            final data = getdata['data'];
            if (widget.update!) {
              final User value = User.fromAddress(data[0]);
              addressList[widget.index!] = value;
              if (checkedDefault.toString() == 'true' ||
                  addressList.length == 1) {
                for (final User i in addressList) {
                  i.isDefault = '0';
                }
